...
export function AllocationClient({ data }: AllocationClientProps) {
    const { bySet, byColor, byType, byRarity, totalValue, setCompletion } = data;
        ...
        return (
            <main className="flex-1 md:ml-64 p-8 pb-20 md:pb-8">
                <EmptyState
                    ...
                    description="Add cards to your portfolio to unlock diversification charts and set completion insights."
    ...
    return (
        <main className="flex-1 md:ml-64 p-8 overflow-y-auto pb-20 md:pb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {/* Set Completion Master Sets */}
                <div className="md:col-span-2 bg-card rounded-xl border border-border/50 shadow-sm p-6">
                                ...
                                <Link href={`/app/sets/${encodeURIComponent(set.name)}`} key={set.name} className="block group">
                                    <div className="flex justify-between text-sm mb-2 group-hover:text-primary transition-colors">
                                        ...
                                        <span className="text-muted-foreground text-xs font-mono group-hover:text-primary/80 transition-colors">
                                            {set.uniqueOwned} / {set.totalInSet} ({set.percentage}%)
                                        </span>
                                    </div>
                                    <div className="w-full h-3 bg-muted rounded-full overflow-hidden">
                                        <div 
                                            className="h-full bg-gradient-to-r from-blue-500 to-purple-500 rounded-full transition-all duration-1000 group-hover:brightness-110"
                                            style={{ width: `${Math.min(set.percentage, 100)}%` }}
                                        />
                                    </div>
                                </Link>
                            ))
                        ) : (
                            <p className="text-center text-muted-foreground text-sm">No set data available.</p>
                        )}
                    </div>
                </div>
            </div>
